...
char str_fhd[1600][100], str_data[1600][10000];
char str_sum[1600][20000], str_out[1600][10000];
int timefile_counts;
...
//   y[0] = hpi - glat(y[0]);
	...
	MPI_Datatype str_rowtype1, str_rowtype2, str_rowtype3;
	MPI_Type_contiguous(100, MPI_CHAR, &str_rowtype1);
	MPI_Type_contiguous(10000, MPI_CHAR, &str_rowtype2);
	MPI_Type_contiguous(20000, MPI_CHAR, &str_rowtype3);
	MPI_Type_commit(&str_rowtype1);
	MPI_Type_commit(&str_rowtype2);
	MPI_Type_commit(&str_rowtype3);
	...
	//   read in event header
		...
		//MPI_Bcast(&timefile_counts, 0, MPI_INT, rank, MPI_COMM_WORLD);

	}
	MPI_Bcast(&total_earthquakes, 1, MPI_INT, 0, MPI_COMM_WORLD);
	timefile_counts = read_timefiles(iread, nxyz, timefiles);
...
//qsort(timefiles, timefile_counts, sizeof(timefiles[0]), (int (*)(const void*, const void*))strcmp);
	...
	*/
int task_index = ceil( (float) total_earthquakes / (float) numtasks);
int nev=0;

//#pragma omp parallel for

for (int nev_task = 0 ; ( nev_task < task_index ) &&  ( nev < total_earthquakes-1 ) ; nev_task++) {
	nev = nev_task + task_index * rank;
	char filename[100];
	sprintf(filename, "%s/%d.eqk", eqkdir, nev + 1);
	FILE *fp_leq = fopen(filename, "r");
	if (!fp_leq) {
		printf("file not found: %s\n", filename);
		printf("%d %d \n",total_earthquakes ,nev);
		assert(0);
	}
	char str_inp[100];
	...
	if (ngood >= nthres) {
		...
		if (stdmin <= stdmax) {
			...
			sprintf(tmp,
					...
					stdmin);
			int len_str_data = 0;
			strapp(str_data[nev_task], &len_str_data, tmp);
			strcpy(str_fhd[nev_task], tmp);
			...
			if (ngood < nsta) {
				strapp(str_out[nev_task], &len_str_out, tmp);
			}
			if(DEBUG_PRINT) {
				printf("\n");
				printf(tmp);
			...
			int len_str_sum = 0;
			strapp(str_sum[nev_task], &len_str_sum, "\n\n");
			sprintf(tmp, "  Minimum Standard Deviation   : %12.8lf\n",
					stdmin);
			strapp(str_sum[nev_task], &len_str_sum, tmp);
			...
			strapp(str_data[nev_task], &len_str_data, "\n");
		}
	} else {
		double ot = dpot + avrmin;
		etoh(ot, &iyr, &jday, &ihr, &imn, &dsec);
		...
		strapp(str_out[nev_task], &len_str_out, "\n");
	}
...
MPI_Gather(str_fhd, task_index, str_rowtype1, str_fhd, task_index, str_rowtype1, 0, MPI_COMM_WORLD);

MPI_Gather(str_data, task_index, str_rowtype2, str_data, task_index, str_rowtype2, 0, MPI_COMM_WORLD);
MPI_Gather(str_sum, task_index, str_rowtype3, str_sum, task_index, str_rowtype3, 0, MPI_COMM_WORLD);
MPI_Gather(str_out, task_index, str_rowtype2, str_out, task_index, str_rowtype2, 0, MPI_COMM_WORLD);
MPI_Finalize();
